...
public class itsoRequestsFragment extends Fragment {
    ...
    private final List<DocumentSnapshot> itsoRequestList = new ArrayList<>();
    ...
    public itsoRequestsFragment() {
        super(R.layout.fragment_itso_requests);
    }
    ...
    private void listenForITSORequests() {
        if (requestsListener != null) {
            requestsListener.remove();
            requestsListener = null;
        }
        ...
        requestsListener = db.collection("requests")
                .addSnapshotListener((queryDocumentSnapshots, error) -> {
                    ...
                    itsoRequestList.clear();

                    for (DocumentSnapshot doc : queryDocumentSnapshots.getDocuments()) {
                        if (isITSORequest(doc) && RequestDataHelper.shouldShowInRequestList(doc)) {
                            itsoRequestList.add(doc);
                        }
                    }
    ...
    private void renderRequestList() {
        ...
        for (DocumentSnapshot doc : itsoRequestList) {
        ...
        if (filteredList.isEmpty()) {
            showEmptyState(
                    ...
                    "There are no ITSO requests under this selected filter."
            );
            return;
        }
